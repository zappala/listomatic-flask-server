...
class ItemTests(unittest.TestCase):
    ...
    def test_items(self):
        ...
        r = test_app.post('/items',data=json.dumps({'item':{'title':'todo item 1'}}),headers=authorization)
        eq_(r.status_code,200)

        # create an item
        #r = test_app.post('/items',data={'text':'todo item 2'},headers=authorization)
        r = test_app.post('/items',data=json.dumps({'item':{'title':'todo item 2'}}),headers=authorization)
        eq_(r.status_code,200)
        ...
        eq_(r.status_code,200)
        assert 'items' in r.data
        eq_(json.loads(r.data)['items'],[{'title':'todo item 1','completed':False,'uri':'/items/1'},{'title':'todo item 2','completed':False,'uri':'/items/2'}])

        # get one item
        r = test_app.get('/items/1',headers=authorization)
        eq_(r.status_code,200)
        eq_(json.loads(r.data),{'title':'todo item 1','completed':False,'uri':'/items/1'})
        ...
        r = test_app.put('/items/1',data={'title':'todo item changed','completed':True},headers=authorization)
        eq_(r.status_code,200)
        eq_(json.loads(r.data),{'title':'todo item changed','completed':True,'uri':'/items/1'})
